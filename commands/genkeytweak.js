...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('genkeytweak')
    .setDescription('Generiert einen Key für LR Tweaker und sendet ihn an den User')
  ...
  async execute(interaction) {
    ...
    try {
      const url = `https://keyauth.win/api/seller/?sellerkey=${config.sellerKeyTweak}&type=add&expiry=${expiryDays}&mask=LR-******-******-******-******&level=${level}&amount=1&format=text`;
      ...
      try {
        await targetUser.send(
          `🔑 **LR Tweaker Lizenzschlüssel**\n\n` +
          `Hallo ${targetUser},\n` +
          `hier ist dein Lizenzschlüssel für den **LR Tweaker V2**:\n` +
          `\`${generatedKey}\`\n\n` +
